...
def main():
 parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('--workers',type=int,default=1);parser.add_argument('--dataset-id',type=int,action='append');parser.add_argument('--output',type=Path,default=Path('runs/replay-published-v1'));args=parser.parse_args()
 ...
 jobs=[(sp,m,r) for sp in datasets for m in METHODS for r in range(10)];cpus=physical_cores()[:args.workers];start=time.perf_counter();results=[]
