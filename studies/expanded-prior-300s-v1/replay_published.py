...
def main():
 parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('--workers',type=int,default=1);parser.add_argument('--dataset-id',type=int,action='append');parser.add_argument('--output',type=Path,default=Path('runs/replay-published-300s-v1'));args=parser.parse_args()
 ...
 verify_freeze();data=ROOT/'data/heldout-expanded-v1';datasets=read(data/'datasets.json');configs=read(data/'configurations.json');table=pd.read_parquet(data/'evaluations.parquet').set_index('record_id')
 replacement=pd.read_parquet(ROOT/'data/heldout-expanded-300s-v1/replacement_evaluations.parquet').set_index('record_id')
 assert set(replacement.index)<=set(table.index)
 for column in ['status','validation_loss','total_cpu_seconds']:table.loc[replacement.index,column]=replacement[column]
 if args.dataset_id:
 ...
 write(RUN/'outcomes.json',truth);write(RUN/'collection.json',read(ROOT/'data/heldout-expanded-300s-v1/collection.json'))
