...
def worker(job):
    import collector
    sp, config, identity, fit_cpu_seconds = job
    collector.FIT_CPU_CAP = fit_cpu_seconds
    return collector.evaluate(sp, config, identity)
...
def main():
    ...
    parser.add_argument('--output', type=Path, default=ROOT / 'runs/collection')
    parser.add_argument('--cache', type=Path, default=ROOT / 'runs/openml-cache')
    parser.add_argument('--prepare-only', action='store_true')
    defaults = json.loads((ROOT / 'collection_defaults.json').read_text())
    parser.add_argument('--fit-cpu-seconds', type=float, default=defaults['fit_cpu_seconds'], help='Training CPU limit per fit; default from collection_defaults.json (300 seconds)')
    args = parser.parse_args()
    if not 0 < args.fit_cpu_seconds < float('inf'):
        parser.error('--fit-cpu-seconds must be positive and finite')
    ...
    environment.update(fit_cpu_seconds=args.fit_cpu_seconds)
    ...
    with ProcessPoolExecutor(max_workers=args.workers, mp_context=mp.get_context('spawn')) as pool:
        for sp in datasets:
            ...
            jobs = [(prepared, c, key([identity, sp['data_identity']]), args.fit_cpu_seconds) for c in configs]
