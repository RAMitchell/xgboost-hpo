...
R=Path(os.environ.get('HPO_RUN_DIR', 'runs/reproduction')).resolve(); S=R.parent
import numpy as np
import pandas as pd
import scipy
...
def matrices(sp,max_bin,threads=1):
    ...
    d=Path(sp['data_dir']);frames={n:pd.read_parquet(d/f'{n}_X.parquet') for n in ['train','validation']};ys={n:np.load(d/f'{n}_y.npy') for n in frames}
...
def evaluate(sp,c,identity,group='records',threads=1,suffix='',round_cap=ROUND_CAP):
    ...
    try:
        ...
        train=np.asarray(history['train'][metric]);val=np.asarray(history['validation'][metric]);reported_best=int(bst.best_iteration);trained=int(bst.num_boosted_rounds())
        checks=dict(curve_length=len(val)==trained,finite_validation=bool(np.isfinite(val).all()),finite_train=bool(np.isfinite(train).all()))
        if not all(checks.values()): raise ValueError(f'Invalid curves: {checks}')
        best=int(np.argmin(val))
        if not guard.stopped and reported_best!=best: raise ValueError(f'best_iteration={reported_best}, curve_argmin={best}')
